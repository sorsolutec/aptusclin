...
import { tenantConfig, TenantConfig } from '@/lib/tenant';

export interface UnitModernPageProps {
  unitId: string;
  initialData?: Partial<TenantConfig>;
}

export default function UnitModernPage({ unitId, initialData }: UnitModernPageProps) {
  const fallback = tenantConfig[unitId] || tenantConfig['sorriso'];

  const data: TenantConfig = {
    ...fallback,
    ...initialData,
    id: initialData?.id || fallback.id,
    nome: initialData?.nome || fallback.nome,
    cidade: initialData?.cidade || fallback.cidade,
    estado: initialData?.estado || fallback.estado,
    endereco: initialData?.endereco || fallback.endereco,
    telefone: initialData?.telefone || fallback.telefone,
    telefoneFixo: initialData?.telefoneFixo || fallback.telefoneFixo || fallback.telefone,
    email: initialData?.email || fallback.email,
    horario: initialData?.horario || fallback.horario || 'Segunda a Sexta: 07:00–11:00, 13:00–17:00',
    cnpj: initialData?.cnpj || fallback.cnpj,
    cnes: initialData?.cnes || fallback.cnes,
  ...
  const whatsappLink = `https://wa.me/${whatsappClean}?text=${encodeURIComponent(
    `Olá! Gostaria de agendar exames ocupacionais na unidade Aptus Clin de ${data.cidade}.`
  )}`;

  const encodedAddress = encodeURIComponent(`${data.endereco || data.cidade} MT`);
  ...
  const faqs = [
    {
      q: `Qual o horário ideal para a realização dos exames admissionais em ${data.cidade}?`,
      a: `Recomendamos o comparecimento pela manhã a partir do início do expediente (${(data.horario || '').split(',')[0] || data.horario || '07:00'}), especialmente para exames laboratoriais de sangue (que necessitam de jejum) e audiometria ocupacional.`,
    },
    {
      q: 'Como funciona o envio dos eventos de SST ao eSocial (S-2210, S-2220 e S-2240)?',
      a: 'Nossa equipe técnica gera e transmite os eventos de Saúde e Segurança do Trabalho diretamente ao portal do eSocial via integração com procuração eletrônica ou fornecimento dos arquivos XML para o sistema contábil da sua empresa.',
    },
    {
      q: 'Quais documentos o colaborador deve levar no dia do exame?',
      a: 'O colaborador deve comparecer com documento oficial com foto (RG ou CNH), CPF e a Guia de Encaminhamento fornecida pela empresa com a função e riscos ocupacionais detalhados.',
    },
    {
      q: `A unidade de ${data.cidade} realiza exames complementares no próprio local?`,
      a: 'Sim! Realizamos exames complementares como audiometria, espirometria, eletrocardiograma (ECG), eletroencefalograma (EEG), acuidade visual e coleta laboratorial/toxicológica no próprio local, evitando que o trabalhador precise se deslocar.',
    },
    {
      q: `Vocês elaboram laudos como PGR, PCMSO e LTCAT para empresas da região de ${data.cidade}?`,
      a: `Sim. Atendemos com excelência empresas do agronegócio, fazendas, cooperativas, indústrias, comércio e serviços de ${data.cidade} e cidades vizinhas, com total conformidade jurídica com as NRs do Ministério do Trabalho.`,
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-700 selection:bg-emerald-500 selection:text-white">
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1">
            {data.cnpj && (
              <span className="inline-flex items-center gap-1.5 text-emerald-400 font-semibold">
                <Shield className="w-3.5 h-3.5" /> CNPJ: {data.cnpj}
              </span>
            )}
            {data.cnes && (
              <>
                <span className="text-slate-400">•</span>
                <span className="text-slate-300">CNES: {data.cnes}</span>
              </>
            )}
            <span className="text-slate-400">•</span>
            <span className="text-blue-200">{data.cidade} – {data.estado}</span>
          </div>

          <div className="flex items-center gap-4 text-slate-300">
            {data.telefoneFixo && (
              <a
                href={`tel:${data.telefoneFixo.replace(/\D/g, '')}`}
                className="hover:text-white transition flex items-center gap-1"
              >
                <Phone className="w-3.5 h-3.5 text-blue-300" /> {data.telefoneFixo}
              </a>
            )}
            {data.telefone && (
              <a
              ...
              >
                <MessageSquare className="w-3.5 h-3.5" /> WhatsApp: {data.telefone}
              </a>
            )}
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto relative z-10 grid lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-7 text-center lg:text-left">
            <div className="inline-flex items-center gap-2 bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 px-3 py-1 rounded-full text-xs font-bold mb-5 tracking-wide">
              ...
              <span>Unidade Especializada em {data.cidade} – {data.estado}</span>
            </div>
            ...
            <p className="text-slate-300 text-sm sm:text-base mt-4 max-w-xl leading-relaxed">
              Atendimento ágil em exames admissionais, periódicos e demissionais, emissão célere de ASO, laudos técnicos (PGR, PCMSO, LTCAT) e envio automatizado dos eventos de SST ao eSocial em {data.cidade}.
            </p>
            ...
            <div className="mt-8 pt-6 border-t border-white/10 grid grid-cols-3 gap-3 text-left">
              <div>
                <span className="text-xs font-extrabold text-emerald-400 block">{(data.horario || '').split(',')[0] || 'Seg a Sex'}</span>
                <span className="text-[11px] text-slate-300">Atendimento ágil para contratações</span>
              </div>
              <div>
                <span className="text-xs font-extrabold text-teal-300 block">ASO no Dia</span>
                <span className="text-[11px] text-slate-300">Agilidade para admissões prioritárias</span>
              </div>
              <div>
            ...
            <div className="bg-white text-slate-800 rounded-3xl p-6 sm:p-7 shadow-2xl border border-slate-100 relative">
              ...
              <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                <div>
                  ...
                  <h3 className="text-lg font-black text-[#002855]">Unidade {data.cidade}</h3>
                </div>
              ...
              <div className="mt-5 space-y-4 text-xs sm:text-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 leading-snug">{data.endereco || `${data.cidade} – ${data.estado}`}</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 font-medium">{data.horario}</p>
                    <p className="text-[11px] text-emerald-600 font-semibold mt-0.5">
                      Coleta matutina para exames em jejum
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {data.telefone && <p className="text-slate-700 font-medium">WhatsApp / Tel: {data.telefone}</p>}
                    {data.telefoneFixo && data.telefoneFixo !== data.telefone && (
                      <p className="text-slate-500 text-xs">Fixo: {data.telefoneFixo}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 pt-4 border-t border-slate-100">
                ...
                >
                  <MessageSquare className="w-4 h-4" /> Falar com Atendente em {data.cidade}
                </a>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center max-w-2xl mx-auto mb-14">
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-[#002855] mt-3">
              Soluções Integradas em Medicina do Trabalho em {data.cidade}
            </h2>
            <p className="text-slate-500 text-sm mt-2">
              Estrutura moderna e equipe técnica qualificada para atender a todos os requisitos das Normas Regulamentadoras (NRs) do Ministério do Trabalho.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-2xl p-6 hover:shadow-xl hover:border-blue-200 transition-all group flex flex-col justify-between">
              ...
              <div className="mt-6 pt-4 border-t border-slate-200">
                ...
                >
                  <span>Agendar ASO em {data.cidade}</span>
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-2xl p-6 hover:shadow-xl hover:border-emerald-200 transition-all group flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 mb-4 leading-relaxed">
                  Estrutura técnica com equipamentos calibrados para a realização imediata dos exames exigidos pelo PCMSO da sua empresa.
                </p>
                <ul className="space-y-2 text-xs text-slate-700 font-medium">
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0" />
                  ...
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0" />
                    <span><strong>Acuidade Visual:</strong> Avaliação para motoristas e operadores</span>
                  </li>
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0" />
          ...
          <div className="grid lg:grid-cols-12 gap-10 items-center">
            <div className="lg:col-span-6">
              ...
              <h2 className="text-2xl sm:text-3xl font-black mt-4 leading-tight">
                Gestão e Envio dos Eventos de SST ao eSocial em {data.cidade}
              </h2>
              <p className="text-slate-300 text-sm mt-3 leading-relaxed">
                Evite multas e autuações da Receita Federal e Ministério do Trabalho. A unidade Aptus Clin {data.cidade} cuida de toda a cadeia de dados ocupacionais com validação e envio digital seguro.
              </p>
            ...
            <div className="lg:col-span-6">
              <div className="bg-gradient-to-br from-slate-800 to-slate-850 p-6 sm:p-8 rounded-3xl border border-slate-700 shadow-2xl">
                <h3 className="text-base font-bold text-emerald-400 flex items-center gap-2">
                  <CheckCircle2 className="w-5 h-5" /> Por que confiar na Aptus Clin {data.cidade}?
                </h3>
                <div className="mt-4 space-y-4 text-xs sm:text-sm text-slate-300">
                  ...
                  <p>
                    ✓ <strong>Atendimento Regional:</strong> Expertise com fazendas, armazéns, transportadoras e comércio local de {data.cidade}.
                  </p>
                </div>
      ...
      {/* ── 6. GUIA DE ORIENTAÇÕES PARA O DIA DO EXAME ── */}
      <section id="orientacoes" className="py-16 px-4 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center max-w-2xl mx-auto mb-12">
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-[#002855] mt-3">
              Orientações para o Dia do Exame em {data.cidade}
            </h2>
            <p className="text-slate-500 text-sm mt-2">
              Compartilhe com seus colaboradores para agilizar o atendimento e evitar remarcações.
            </p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                  Para exames de sangue (glicemia, lipidograma), é necessário jejum de 8 a 12 horas. Água pode ser consumida com moderação.
                </p>
              </div>
              <span className="text-[11px] font-bold text-amber-700 mt-4 block">Coleta matutina</span>
            </div>
            ...
            <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                  Exige repouso auditivo obrigatório de no mínimo 14 horas (evitar uso de fones de ouvido e som automotivo alto antes do teste).
                </p>
              </div>
            ...
            <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                  Caso utilize óculos de grau ou lentes de contato corretivas, é indispensável trazê-los para a avaliação.
                </p>
              </div>
      ...
      {/* ── 7. SEÇÃO DE LOCALIZAÇÃO INTERATIVA ── */}
          ...
          <div className="grid lg:grid-cols-12 gap-10 items-center">
            <div className="lg:col-span-5">
              ...
              <h2 className="text-2xl sm:text-3xl font-black text-[#002855] mt-2">
                Onde Fica a Unidade de {data.cidade}
              </h2>
              <p className="text-slate-600 text-sm mt-3 leading-relaxed">
                Ponto de atendimento com fácil acesso e estacionamento para trabalhadores e colaboradores da região de {data.cidade} – {data.estado}.
              </p>
              ...
              <div className="mt-6 space-y-4 text-xs sm:text-sm">
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                  ...
                  <p className="text-slate-700 mt-1">{data.endereco || `${data.cidade} – ${data.estado}`}</p>
                  <p className="text-slate-400 text-xs mt-0.5">{data.cidade} – {data.estado}</p>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                  ...
                  <p className="text-slate-700 mt-1">{data.horario}</p>
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                  ...
                  <p className="text-slate-700 mt-1">
                    WhatsApp: <strong>{data.telefone}</strong> {data.telefoneFixo && `| Fixo: ${data.telefoneFixo}`}
                  </p>
                  <p className="text-slate-500 text-xs mt-0.5">E-mail: {data.email}</p>
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
                <div className="bg-gradient-to-r from-[#002855] to-[#04336c] p-4 text-white flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-xs sm:text-sm">{data.endereco || data.cidade}</span>
                  </div>
                ...
                <div className="relative w-full h-80 sm:h-96 bg-slate-100">
                  <iframe
                    title={`Mapa Aptus Clin ${data.cidade}`}
                    width="100%"
                    height="100%"
                    className="border-0"
                    loading="lazy"
                ...
                <div className="p-4 bg-slate-50 flex flex-wrap items-center justify-between text-xs text-slate-500">
                  <span>Fácil acesso para colaboradores e frotas de empresas</span>
                  <a
                    href={whatsappLink}
                    target="_blank"
                    rel="noopener noreferrer"
      ...
      {/* ── 8. FORMULÁRIO DE PROPOSTA CORPORATIVA ── */}
          ...
          <div className="bg-gradient-to-br from-[#002855] to-[#052b57] text-white rounded-3xl p-8 sm:p-10 shadow-2xl relative overflow-hidden">
            ...
            <div className="text-center max-w-xl mx-auto mb-8 relative z-10">
              ...
              <h2 className="text-2xl sm:text-3xl font-black mt-2">
                Solicite uma Proposta para a sua Empresa em {data.cidade}
              </h2>
              <p className="text-slate-300 text-xs sm:text-sm mt-2">
                Gestão de saúde ocupacional sob medida para o tamanho da sua equipe. Receba contato imediato da equipe de {data.cidade}.
              </p>
            </div>
      ...
      <section className="py-16 px-4 bg-slate-50">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-[#002855] mt-2">
              Perguntas Frequentes sobre Medicina do Trabalho em {data.cidade}
            </h2>
          </div>
        ...
        <div className="max-w-6xl mx-auto grid sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
          <div>
            ...
            <p className="text-slate-400 mt-3 leading-relaxed">
              Referência regional em Medicina Ocupacional, Engenharia de Segurança e Gestão de eSocial em Mato Grosso.
            </p>
            <div className="mt-4 flex items-center gap-3">
              ...
              {data.facebook && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 text-blue-400 flex items-center justify-center transition"
                  title="Facebook"
                >
          ...
          <div>
            <h4 className="font-black text-white uppercase tracking-wider text-xs mb-3">Unidade {data.cidade} – {data.estado}</h4>
            <p className="text-slate-300 leading-snug">{data.endereco || `${data.cidade} – ${data.estado}`}</p>
            {data.cnpj && <p className="mt-2 text-slate-400">CNPJ: {data.cnpj}</p>}
            {data.cnes && <p className="text-slate-400">CNES: {data.cnes}</p>}
          </div>

          <div>
            <h4 className="font-black text-white uppercase tracking-wider text-xs mb-3">Horário & Contatos</h4>
            <p className="text-slate-300">{data.horario}</p>
            {data.telefone && <p className="mt-2 text-slate-300">WhatsApp: {data.telefone}</p>}
            {data.telefoneFixo && <p className="text-slate-300">Fixo: {data.telefoneFixo}</p>}
            {data.email && <p className="text-slate-300">E-mail: {data.email}</p>}
          </div>
      ...
      <a
        ...
        title={`Falar com a Aptus Clin ${data.cidade} no WhatsApp`}
      >
